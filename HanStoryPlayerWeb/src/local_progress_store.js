...
export async function saveBackup(label,records){records??=await readSnapshot();const backup={id:`backup:${label}:${new Date().toISOString()}`,label,createdAt:new Date().toISOString(),records};await put('progressBackups',backup,{sync:false});return backup;}
export async function saveOwnerSnapshot(owner,records){records??=await readSnapshot();return put('progressBackups',{id:`owner:${owner}`,label:owner,createdAt:new Date().toISOString(),records},{sync:false});}
export async function ownerSnapshot(owner){return(await get('progressBackups',`owner:${owner}`))?.records||[];}
export async function progressSummary(records){records??=await readSnapshot();let xp=0,completed=new Set(),latest='';for(const record of records){xp=Math.max(xp,Number(record.value?.xp||0));for(const id of record.value?.completedLessons||[])completed.add(id);const date=record.updatedAt||record.value?.lastDate||record.value?.lastStudyDate||'';if(date>latest)latest=date;}return{xp,completedLessons:completed.size,lastActivity:latest||'Sin actividad'};}

...
export class AuthService extends EventTarget{
  ...
  async signInWithGoogle(){if(!this.client)throw new Error('Configura Supabase antes de iniciar sesión.');sessionStorage.setItem(AUTH_RETURN_KEY,location.hash||'#/');const{error}=await this.client.auth.signInWithOAuth({provider:'google',options:{redirectTo:getAuthRedirectUrl(),scopes:'openid email profile'}});if(error)throw error;}

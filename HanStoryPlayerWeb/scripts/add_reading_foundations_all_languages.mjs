  ...
  English:{
    ...
    lessons:[
      L('Letras, nombres y sonidos','Comprende por qué saber el abecedario no basta para leer.',
        C('La letra no es su nombre','A · /æ/','ay','A se llama «ei», pero dentro de una palabra puede representar otro sonido.','Pregunta siempre: ¿nombre de letra o sonido dentro de palabra?','En inglés una misma letra puede representar sonidos distintos según la palabra.',['A es la letra escrita.','«ei» es su nombre.','El sonido se aprende dentro de palabras.'],'¿Qué debes mirar al leer una palabra?','El sonido de la letra en esa palabra',['Solo el nombre del abecedario','La traducción española'],[{label:'NOMBRE DE LA LETRA',text:'A',meaning:'se llama «ei»',audio:'ay'},{label:'DENTRO DE UNA PALABRA',text:'car',meaning:'auto',audio:'car'}]),
        C('El contexto decide','C: car / city','see','C puede sonar como k o como s.','Mira la vocal que aparece después.','Muchas correspondencias inglesas dependen de las letras vecinas.',['c + a/o/u suele sonar k.','c + e/i/y suele sonar s.','Aprende patrones, no reglas absolutas.'],'¿Puede C tener más de un sonido?','Sí, depende del contexto',['No, siempre suena igual','Solo en nombres propios'],[{label:'C CON SONIDO K',text:'car',meaning:'auto',audio:'car'},{label:'C CON SONIDO S',text:'city',meaning:'ciudad',audio:'city'}])),
      L('Las vocales','Distingue vocales breves, largas y combinaciones frecuentes.',
        ...
        C('Vocal larga y e silenciosa','a_e: name','ay','En muchos patrones consonante + e final, la primera vocal cambia.','La e final suele modificar y no sonar.','El patrón de la e silenciosa aparece en muchas palabras frecuentes.',['En name y cake, la a tiene sonido largo.','La e final suele ser silenciosa.','Hay excepciones que se aprenden con uso.'],'¿Qué hace normalmente la e final en name?','Modifica la vocal anterior y no suena',['Se pronuncia como e española','Duplica la consonante'],[{label:'A LARGA + E SILENCIOSA',text:'name',meaning:'nombre',audio:'name'},{label:'OTRO EJEMPLO',text:'cake',meaning:'pastel',audio:'cake'}])),
      L('Consonantes que confunden','Separa sonidos ingleses que un hispanohablante suele mezclar.',
        C('B y V no son iguales','B / V','vee','V se articula con dientes superiores y labio inferior.','No conviertas automáticamente V en B.','La diferencia puede cambiar una palabra.',['B usa los dos labios.','V usa dientes y labio inferior.','Escucha y observa la articulación.'],'¿B y V representan el mismo sonido inglés?','No, se articulan de manera distinta',['Sí, siempre','Solo cambian al escribir'],[{label:'SONIDO B',text:'book',meaning:'libro',audio:'book'},{label:'SONIDO V',text:'very',meaning:'muy',audio:'very'}]),
  ...
  French:{
    ...
    lessons:[
      ...
      L('Vocales claras','Une una consonante conocida con una sola vocal.',
        C('A dentro de una sílaba','a → ma','ma','Escucha la vocal a dentro de ma.','Mira a mientras oyes ma.','La consonante permite escuchar la vocal dentro de una sílaba natural. Concéntrate en el sonido central: a.',['m + a → ma.','La vocal central es a.','No se presenta vocabulario todavía.'],'¿Qué vocal aparece en la sílaba ma?','a',['i','o']),
